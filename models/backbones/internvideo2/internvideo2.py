...
class StandardMLP(nn.Module):
    def __init__(self, in_features, hidden_features, dropout_prob=0.1):
        super(StandardMLP, self).__init__()
        # 第一个全连接层：从 in_features 到 hidden_features
        self.fc1 = nn.Linear(in_features, hidden_features)
        # 第二个全连接层：从 hidden_features 回到 in_features
...
class DropoutAddRMSNorm(nn.Module):
    def __init__(self, hidden_size: int, eps: float = 1e-6, dropout_prob=0.1) -> None:
        super().__init__()
        self.eps = eps
    ...
    def forward(self, hidden_states, residual=None, return_residule=False):
        # 先做dropout
        dropped = self.dropout(hidden_states)
        # 做残差连接
        if residual is not None:
            residual = residual + dropped
        else:
            residual = dropped
        # RMSNorm
        normed = self.weight * self._norm(residual.float()).type_as(residual)
        # 返回norm后的结果和residual
        if return_residule:
...
# class DropoutAddRMSNorm(nn.Module):
#     def __init__(self, hidden_size, dropout_prob=0.1, eps=1e-6):
#         super(DropoutAddRMSNorm, self).__init__()
#         self.dropout = nn.Dropout(dropout_prob)
#         self.rms_norm = nn.LayerNorm(hidden_size, eps=eps)
...
try:
    ...
    # from ..vit.clip import LayerNorm
    # DropoutAddRMSNorm = DropoutAddRMSNorm
...
class Attention(nn.Module):
    ...
    def _flash_attn(self, x, key_padding_mask=None, need_weights=False):
        ...
        context, _, attn = self.inner_attn(
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        if use_fused_mlp:
            self.mlp = FusedMLP(in_features=dim, hidden_features=mlp_hidden_dim)
        else:
    ...
    def forward(self, x, residual=None):
        ...
        def _inner_forward(x, residual=None):
            if self.use_fused_rmsnorm:
                x, residual = self.norm1(x, residual=residual, return_residule=True)
                x, attn = self.attn(x)
                x = self.drop_path1(self.ls1(x))
                x, residual = self.norm2(x, residual=residual, return_residule=True)
...
class PretrainInternVideo2(nn.Module):
    def __init__(
        ...
        if use_fused_rmsnorm:
            norm_layer_for_blocks = partial(DropoutAddRMSNorm, eps=1e-6)
        else:
    ...
    def forward(self, x, mask=None, use_image=False, x_vis_return_idx=-1, x_vis_only=False):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.return_index:
                if isinstance(x, tuple) and len(x) == 3:
                    tmp_x, tmp_residual, attn = x
                    if residual is not None:
                        tmp_x = tmp_x + tmp_residual
                    # else:
                    tmp_x = rearrange(tmp_x[:,1:], "b (t n) d -> (b t) d n", t=4)
                    tmp_x = torch.nn.AdaptiveMaxPool1d(196)(tmp_x)
                    tmp_x = rearrange(tmp_x, "(b t) d n -> (b t) n d", t=4)
                    tmp_x = torch.nn.AdaptiveMaxPool1d(512)(tmp_x)
                    tmp_x = rearrange(tmp_x, "(b t) n d -> b (t n) d", t=4)
                    x_clip.append(tmp_x)
        ...
        vdo_feature = rearrange(vdo_feature, "b (t n) d -> (b t) d n", t=4)
        vdo_feature = torch.nn.AdaptiveMaxPool1d(196)(vdo_feature)
        vdo_feature = rearrange(vdo_feature, "(b t) d n -> (b t) n d", t=4)
        vdo_feature = torch.nn.AdaptiveMaxPool1d(768)(vdo_feature)
        vdo_feature = rearrange(vdo_feature, "(b t) n d -> b (t n) d", t=4)

        attn = rearrange(attn.mean(1)[:,0,1:], "b (t n) -> (b t) n", t=4)
        attn = torch.nn.AdaptiveMaxPool1d(196)(attn)

        return x_clip, attn, vdo_feature
...
def pretrain_internvideo2_1b_patch14_224(config):
    model = PretrainInternVideo2(
        ...
        clip_teacher_embed_dim=config.vision_encoder.clip_output_dim,
        clip_teacher_final_dim=config.vision_encoder.clip_output_dim,
        clip_norm_type='l2',
        clip_return_layer=config.vision_encoder.clip_return_layer,
        clip_student_return_interval=1,
    )
    pretrain_pth = "/YOUR_PATH/projects/stm/multi_modality/pretraining_models/InternVideo2-stage2_1b-224p-f4.pt"
    if pretrain_pth is not None:
        logger.info(f"Loading pretrained weights from {pretrain_pth}")
        ...
        for key, value in state_dict['module'].items():
            ...
            if key.startswith(prefix + '.'):
                ...
                new_key = key  # 如果键名中没有前缀，保持不变
            new_state_dict[new_key] = value
        interpolate_pos_embed_internvideo2(new_state_dict, model, orig_t_size=4)
