...
import sys
sys.path.append("vit/")
# import actionclip_clip as clip
# from actionclip_model import VisualTransformer
from . import build_vit, build_clip
# import clip
import logging
...
def create_actionclip(config, pretrain=True, pretrain_path="pretraining_models/actionclip_vit-b-16-8f.pt"):
    model = build_clip(config)
    ...
    if pretrain:
        if os.path.isfile(pretrain_path):
            logger.info("=> loading checkpoint '{}'".format(pretrain_path))
            ...
            fusion_model = visual_prompt("Transf", checkpoint['model_state_dict'], 4)
            new_state_dict = OrderedDict()
            for k, v in checkpoint['fusion_model_state_dict'].items():
            ...
            logger.info(("=> no checkpoint found at '{}'".format(pretrain_path)))
