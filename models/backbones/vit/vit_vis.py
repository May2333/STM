    ...
    def forward(self, x, return_attn=False):
        if return_attn:
            if self.gamma_1 is None:
                x_, attn = self.attn(self.norm1(x), return_attn=True)
...
def get_sinusoid_encoding_table(n_position, d_hid, ckpt_num_frame=-1, cur_frame=12): 
    ...
    if ckpt_num_frame != -1 and ckpt_num_frame != cur_frame:
        ...
        sinusoid_table = sinusoid_table.permute(0, 2, 3, 4, 1).contiguous().reshape(-1, C, T)  # BHW, C, T
        sinusoid_table = torch.nn.functional.interpolate(sinusoid_table, size=new_T, mode='linear')
        sinusoid_table = sinusoid_table.reshape(1, P, P, C, new_T).permute(0, 4, 1, 2, 3).contiguous() # B, T, H, W, C
...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask=None, use_image=False, return_attn=False, image_mask_type="orimask"):
        ...
        B, N, C = x.shape
        if mask is not None:
            if "temporal" in image_mask_type:
        ...
        for idx, blk in enumerate(self.blocks):
            if self.use_checkpoint and idx < self.checkpoint_num:
                if return_attn:
                    x_vis, attn = checkpoint.checkpoint(blk, x_vis, return_attn=True)
                else:
                    x_vis = checkpoint.checkpoint(blk, x_vis)
            else:
                if return_attn:
                    x_vis, attn = blk(x_vis, return_attn=True)
                else:
                    x_vis = blk(x_vis)
        ...
        if return_attn:
            return x_vis, x_clip_vis, attn
        else:
            return x_vis, x_clip_vis

    def forward(self, x, mask=None, use_image=False, return_attn=False, image_mask_type="orimask"):
        if return_attn:
            x_vis, x_clip_vis, attn = self.forward_features(x, mask, use_image, return_attn, image_mask_type=image_mask_type)
            return x_vis, x_clip_vis, attn
        else:
            x_vis, x_clip_vis = self.forward_features(x, mask, use_image, return_attn, image_mask_type=image_mask_type)
            return x_vis, x_clip_vis
...
class PretrainVisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # if "invisible" in  uta:
        #     self.message_attn = ILA_decoder(T=4, d_model=clip_decoder_embed_dim, patch_size=16,input_resolution=224)
    ...
    def forward(self, x, mask=None, use_image=False, keep_temporal=False, image_mask_type="orimask", uta="visible", decoder='ila', text_proj=None, return_attn=False):
        T = x.shape[2]
        x_vis, x_clip_vis, attn = self.encoder(x, None, use_image, return_attn=True) # [B, N_vis, C_e]
        ...
        # clip_pos_emd_vis = expand_clip_pos_embed.view(B, -1, C_CLIP).unsqueeze(0).repeat(K, 1, 1, 1)
        ...
        x_clip = x_vis # align and normalize
        # else:
        #     x_clip = None

        # x_clip = rearrange(x_clip, 'k b t n c -> k b (t n) c', t=4)
...
#             x_clip = x_clip_vis
        if self.add_pool_norm:
            ...
            if uta == "visible_plus_invisibe":
                return x_vis, x_pool_vis, x_clip, x_clip_invis
            else:
                if return_attn:
                    return x_vis, x_pool_vis, x_clip, attn
                else:
                    return x_vis, x_pool_vis, x_clip
        else:
            if self.uta == "visible_plus_invisibe":
                return x_vis, x_clip, x_clip_invis
            else:
                if return_attn:
                    return x_vis, x_clip, attn
                else:
                    return x_vis, x_clip
        
    def forward_encoder_ori_umt(self, x, mask=None, use_image=False, keep_temporal=False, return_attn=False):
        T = x.shape[2]
        x_vis, pooled_vision_embeds, attn = self.encoder(x, mask, use_image, return_attn) # [B, N_vis, C_e]
        return x_vis, pooled_vision_embeds, attn
