...
inputs = dict(
    ...
    batch_size=dict(image="${batch_size_img}", video="${batch_size}"),
...
model = dict(
    ...
    vision_encoder=dict(
        ...
        # mask
        video_mask_type="temporal",
        video_mask_ratio=0.7,
...
optimizer = dict(
    ...
    different_lr=dict(enable=True, module_names=["lambda_param_tp"], lr=5e-2),
)

scheduler = dict(sched="cosine", epochs=15, min_lr_multi=0.01, warmup_epochs=2)
