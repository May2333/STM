...
def setup_internvideo2(config: dict):
    if "bert" in config.text_encoder.name:
        # tokenizer = BertTokenizer.from_pretrained(config.text_encoder.pretrained, local_files_only=True)
        model = InternVideo2_Stage2(config=config, tokenizer=None, is_pretrain=True)
    else:
    ...
    elif config.get('use_half_precision', True):
...
class InternVideo2_Stage2(nn.Module):
    ...
    def __init__(self, 
        ...
        self.vision_width = config.vision_encoder.clip_decoder_embed_dim
    ...
    def encode_vision(self, 
        ...
        if test:
            vision_embeds, attn, pooled_vision_embeds = self.vision_encoder(
                image, None, use_image)
            return vision_embeds, attn, pooled_vision_embeds
        else:
    ...
    def build_vision_encoder(self):
        ...
        vision_encoder = pretrain_internvideo2_1b_patch14_224(self.config)
        # else:
