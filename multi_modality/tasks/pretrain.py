...
def train(
    ...
    model.train()
    # teacher_params = sum(p.numel() for p in model.clip_teacher.parameters())
    # # teacher_params += sum(p.numel() for p in model.module.fusion_model.parameters())
...
def main(config):
    ...
    model_cls = eval(config.model.get('model_cls', 'STM'))
    (
        model,
        model_without_ddp,
        optimizer,
    ...
    for epoch in range(start_epoch, config.scheduler.epochs):
        ...
        if is_main_process():
            ...
            if config.stop_key is not None and config.stop_key in eval_res:
                ...
                cur_r_mean = best  # save the last as the best
