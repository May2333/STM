...
def _compress_videos(input_output_pair, size=224, fps=3):
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"ffmpeg error on file {input_file_path}:\n{result.stdout.decode('utf-8')}")
    except Exception as e:
        print(f"Exception during compressing {input_file_path}: {e}")
...
def prepare_input_output_pairs(input_root, output_root, input_file_list_path=None):
    ...
    for e in tqdm(filenames, desc="find un-processed videos/images"):
        ...
        # 如果原始文件没有后缀，输出路径依然加上 .mp4
        if '.' in e:
            filename_wo_ext = os.path.splitext(e)[0]
        else:
            filename_wo_ext = e

        output_file_path = join(output_root, filename_wo_ext + ".mp4")
