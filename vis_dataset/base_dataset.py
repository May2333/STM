        ...
        for _ in range(self.num_tries):
            try:
                ...
                frames, frame_indices, video_duration = self.video_reader(
                    data_path, self.num_frames, "middle", 
                    max_num_frames=max_num_frames, client=self.client,
                    trimmed30=self.trimmed30
                )
            except Exception as e:
                with open("/YOUR_PATH/data/umt_data/webvid_2m/Webvid2M/error_mp4.txt", "a") as f:
            ...
            if self.visualization:
                ori_trans = transforms.Compose(
                    [transforms.RandomResizedCrop(size=(224, 224), scale=(0.5, 1.0), ratio=(0.75, 1.3333)),
                    transforms.RandomHorizontalFlip(p=0.5),]
    )
                import cv2
                
#                 import pdb;pdb.set_trace()
#                 image_np = frames[0].cpu().permute(1, 2, 0).mul(255).byte().numpy()
#                 image_cv = cv2.cvtColor(image_np, cv2.COLOR_RGB2BGR)
#                 save_name = os.path.join("hhh.jpg")
#                 cv2.imwrite(save_name, image_cv) 
                ori_frames = ori_trans(frames)
                frames = self.transform(frames)

                return (frames, ori_frames), index
            else:
                frames = self.transform(frames)
                return frames, index
